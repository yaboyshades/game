...
class GameServer:
        ...
        try:
            ...
            if message_type == "message":
                ...
                if not player_character.name:
                    ...
                    await self.handle_character_creation(user_id, player_input)
                else:
                    # Process normal game input
                    await self.process_game_input(user_id, player_input)
            
        except json.JSONDecodeError:
            # Handle plain text input
            logger.warning(f"Received non-JSON message from user {user_id}: {message[:100]}...")
            await self.process_game_input(user_id, message)
        except Exception as e:
    ...
    async def handle_character_creation(self, user_id: str, player_input: str):
    ...
    async def process_game_input(self, user_id: str, player_input: str):
    ...
    async def send_game_state(self, user_id: str):
        """Send current game state to a user."""
        player_character = self.get_player_character(user_id)
        if not player_character:
            return
